...
export class SpringDataServiceService {
  ...
  addDat(arObj){
    ...
    console.log(arObj);
    return (this.httpClient.post("http://localhost:8004/auth/verfication/favMusic/saveFav",arObj));
  }
  getData(artistId):Observable<Array<any>>{
    console.log("Inside retriving data as per artistID");
    return (this.httpClient.get<any>(`http://localhost:8004/auth/verfication/favMusic/getFav/${artistId}`));
  }
  saveRecommendation(recomObj){
    return(this.httpClient.post("http://localhost:8005/auth/verfication/favMusic/addRecom",recomObj));
  }
  getAllMusicList():Observable<Array<any>>{
    console.log("Inside retriving All list");
    return (this.httpClient.get<any>(`http://localhost:8004/auth/verfication/favMusic/getallFav`));
  }
  getAllRecommendation():Observable<Array<any>>{
    console.log("Inside retriving All list");
    return (this.httpClient.get<Array<any>>(`http://localhost:8005/auth/verfication/favMusic/getAllRecom`));
  }
